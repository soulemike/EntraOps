...
BeforeAll {
    $script:EntraOpsBaseFolder = Split-Path $PSScriptRoot -Parent
    . "$PSScriptRoot/../EntraOps/Public/Automation/Resolve-EntraOpsUpdateSource.ps1"
    . "$PSScriptRoot/../EntraOps/Public/Automation/Test-EntraOpsUpdateContract.ps1"
    . "$PSScriptRoot/../EntraOps/Public/Automation/Get-EntraOpsUpdateCandidate.ps1"
    . "$PSScriptRoot/../EntraOps/Public/Automation/Get-EntraOpsUpdatePlan.ps1"
    . "$PSScriptRoot/../EntraOps/Public/Core/Update-EntraOps.ps1"
}
...
$UpdateWorkflowFile = Join-Path $PSScriptRoot '../.github/workflows/Update-EntraOps.yaml'
...
if ([string]::IsNullOrWhiteSpace($CurrentRepositoryFullName)) {
    $OriginUrl = & git -C (Split-Path $PSScriptRoot -Parent) remote get-url origin 2>$null
...
Describe 'Update-EntraOps update scope' {
    ...
    It 'validates the downloaded candidate before replacing any local target' {
        $Source = Get-Content -LiteralPath (Join-Path $PSScriptRoot '../EntraOps/Public/Core/Update-EntraOps.ps1') -Raw
...
Describe 'Generated artifact validation' {
    BeforeAll {
        $PullWorkflow = Get-Content -LiteralPath (Join-Path (Split-Path $PSScriptRoot -Parent) '.github/workflows/Pull-EntraOpsPrivilegedEAM.yaml') -Raw
    }
    ...
    It 'reports contradictory tier pairs as warnings by default' {
        ...
        $ConfigGenerator = Get-Content -LiteralPath (Join-Path (Split-Path $PSScriptRoot -Parent) 'EntraOps/Public/Configuration/New-EntraOpsConfigFile.ps1') -Raw
...
Describe 'Deployment browser validation' {
    It 'runs browser tests without restricting them to the upstream repositories' {
        $Workflow = Get-Content -LiteralPath (Join-Path $PSScriptRoot '../.github/workflows/Test-EntraOps.yaml') -Raw
    ...
    It 'tests generated reports before artifacts are uploaded' {
        $Workflow = Get-Content -LiteralPath (Join-Path $PSScriptRoot '../.github/workflows/Push-EntraOpsPrivilegedReporting.yaml') -Raw
...
Describe 'Git-Push credential header' {
    BeforeAll {
        $script:GitPushAction = Get-Content -LiteralPath (Join-Path $PSScriptRoot '../.github/actions/Git-Push/action.yml') -Raw
        $script:GitPushScript = Get-Content -LiteralPath (Join-Path $PSScriptRoot '../.github/scripts/Publish-EntraOpsGitHubChanges.ps1') -Raw
    }
    ...
    It 'checks native exit codes and distinguishes git diff --quiet results in both publishers' {
        $UpdatePublisher = Get-Content -LiteralPath (Join-Path $PSScriptRoot '../.github/scripts/Publish-EntraOpsGitHubUpdate.ps1') -Raw
...
Describe 'Git-Push publisher behavior' {
    BeforeAll {
        $script:Publisher = Join-Path $PSScriptRoot '../.github/scripts/Publish-EntraOpsGitHubChanges.ps1'
...
Describe 'Workflow job conditions' -Skip:(-not [bool](Get-Module -ListAvailable -Name powershell-yaml)) {
    ...
    It 'never gates a job on the env context' {
        $WorkflowFiles = @(Get-ChildItem -LiteralPath (Join-Path $PSScriptRoot '../.github/workflows') -File |
...
Describe 'Shipped workflow schedule triggers' {
    ...
    It 'ships <_> without a schedule trigger' -ForEach $ScheduledWorkflowFiles -Skip:(-not $IsShippedUpdateWorkflow) {
        $Workflow = Get-Content -LiteralPath (Join-Path $PSScriptRoot '../.github/workflows' $_) -Raw
