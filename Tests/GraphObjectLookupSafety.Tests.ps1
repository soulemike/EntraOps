...
BeforeAll {
    . "$PSScriptRoot/../EntraOps/Private/ConvertTo-EntraOpsODataStringLiteral.ps1"
    . "$PSScriptRoot/../EntraOps/Private/Select-EntraOpsUniqueGraphObject.ps1"
}
...
Describe "Graph object lookup safety" {
    ...
    Context "report tier badge maps cover every tier the generators emit" {
        It "<File> maps WorkloadPlane in its tierBadge implementation" -TestCases @(
            ...
            $Content = Get-Content -Raw -LiteralPath (Join-Path $PSScriptRoot ".." $File)
