/*
 * Privileged EAM Reporting - shared Review list
 *
 * Cross-tool "Review list" for roles, role actions (including their scope) and
 * privileged principals: items can be starred in the Classification Explorer,
 * the EAM Dashboard, the Access Path Map and the Tier Breach Analyzer and are
 * stored in localStorage so the list is
 * shared across all reporting apps served from the same origin. Every item
 * keeps a deep link (app + location hash) that jumps back to the exact place
 * where it was selected.
 *
 * This file is intentionally identical in every sub-app (self-contained apps
 * by design). API: window.EOReview
...
(function () {
    ...
    /*
     * item: { id, kind ('Role'|'Role action'|'Principal'), system, name, scope,
     *         tier, hash (location hash for the deep link) }
    ...
    function linkFor(item) {
        // Apps are sibling folders below Reports/, so relative links keep working
        // wherever the portal is hosted (file://, static web server, ...).
    ...
    function buildUi() {
        ...
        // Keep the count in sync when another tab / app changes the list.
    ...
    function renderPanel() {
        ...
        var html =
            ...
            '<span class="eo-review-sub">' + list.length + " item(s) &middot; shared across all reporting tools</span>" +
        ...
        if (!list.length) {
            html +=
                ...
                "(incl. its scope) or privileged principal in the Classification Explorer, the EAM Dashboard, the " +
                "Access Path Map or the Tier Breach Analyzer to collect it here for review.</div>";
        } else {
            list.slice().reverse().forEach(function (item) {
                var kindChipCls = item.kind === "Role action" ? "docdiff" : item.kind === "Principal" ? "warn" : "brand";
                html +=
